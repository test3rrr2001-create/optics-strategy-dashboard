...
// ストップワード: 一般的なキーワードやドメイン名、ニュースサイト特有の言葉を除外
const STOPWORDS = new Set([
  ...
  "jp", "com", "net", "org", "www", "https", "http", "発売", "開始", "発表", "提供", "リリース", "最新", 
  "国内", "世界", "初", "予定", "実施", "導入", "活用", "展開", "連携", "強化", "推進", "検討", "共同",
  "報告", "調査", "レポート", "市場", "動向", "まとめ", "ニュース", "情報", "一覧", "確認", "詳細"
]);

// 光学機器関連のポジティブキーワード（優先的に抽出したい概念）
const OPTICS_KEYWORDS = [
  "顕微鏡", "内視鏡", "スコープ", "カメラ", "レンズ", "光学", "画像", "診断", "検査", 
  "センサー", "レーザー", "映像", "解析", "デジタル", "モニタ", "医療機器", "非破壊"
];
...
function extractWords(text) {
  ...
  return words.filter(w => {
    ...
    // 1文字だけの言葉、数字だけのもの、ストップワードは除外
    if (word.length < 2) return false;
    if (/^[0-9]+$/.test(word)) return false;
    if (STOPWORDS.has(word)) return false;

    // 光学機器関連の単語が含まれているか、または単語が一定以上の長さ（名詞の塊）であれば採用
    const isOpticsRelated = OPTICS_KEYWORDS.some(k => word.includes(k));
    return isOpticsRelated || word.length >= 3;
  });
}
